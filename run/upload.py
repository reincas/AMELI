...
from ameli.lanthanide.zenodo import ZenodoBucket, get_configs, get_zip_folders
...
def desciption(alt_kwargs=None):
    matrix_rows = []
...
if __name__ == "__main__":
    ...
    for num_electrons in [1]:  # get_configs():
        ...
        ion = f"{element}<sup>3+</sup>"

        title = f"Matrix Elements for the {element} Ion"

        desc = desciption()
        zenodo = ZenodoBucket(title, desc, sandbox=True)

        for zip_name, root_path, files in get_zip_folders(num_electrons):
            response = zenodo.upload_zip(files, root_path, zip_name)
